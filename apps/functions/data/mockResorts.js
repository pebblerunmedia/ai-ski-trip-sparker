...
function calculateSimilarityScore(resort, keywords) {
    const matchingKeywords = resort.keywords.filter(keyword => keywords.some(userKeyword => keyword.toLowerCase().includes(userKeyword.toLowerCase()) ||
...
function searchResorts(query, limit = 3) {
    const queryLower = query.toLowerCase();
    const searchTerms = queryLower.split(' ').filter(term => term.length > 2);
    const scoredResorts = exports.mockSkiResorts.map(resort => {
        let score = 0;
...
//# sourceMappingURL=mockResorts.js.map